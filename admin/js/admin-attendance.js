// admin/js/admin-attendance.js - ADMIN VERSION (can view all classes)
document.addEventListener("DOMContentLoaded", () => {
    ...
    // === OPTIMIZED API FUNCTIONS (ADMIN VERSION) ===
    async function fetchAllClasses() {
        try {
            const cacheKey = 'all_classes';
            if (cache[cacheKey]) return cache[cacheKey];

            const res = await fetch(`../php/get_all_classes.php?bid=${getBrowserInstanceId()}`);
            ...
            showToast("Failed to load classes: " + (data.message || 'Unknown error'), "danger");
            return [];
        } catch (err) {
            console.error("fetchAllClasses error:", err);
            showToast("Error loading classes: " + err.message, "danger");
            return [];
        }
    }
    ...
    async function fetchStudentsByClass(classId) {
        try {
            ...
            const res = await fetch(`../php/get_students_by_class.php?bid=${getBrowserInstanceId()}&class_id=${classId}`);
            const data = await res.json();
            ...
            console.error("fetchStudentsByClass:", err);
    ...
    async function fetchAttendance(classId, date = null) {
        try {
            ...
            const res = await fetch(`../php/get_admin_attendance.php?bid=${getBrowserInstanceId()}&class_id=${classId}&date=${date}`);
            const data = await res.json();
    ...
    function updateClassSelect() {
        ...
        allClasses.forEach(cls => {
            ...
            option.value = cls.id;
            const teacherText = cls.teacher_name ? ` - ${cls.teacher_name}` : '';
            option.textContent = `${cls.class_name} (${cls.year_group})${teacherText}`;
            option.dataset.teacher = cls.teacher_name || 'No Teacher';
            option.dataset.isWeekend = isWeekendClass(cls.class_name);
    ...
    function updateCurrentClassInfo() {
        ...
        currentClassIsWeekend = selectedOption.dataset.isWeekend === 'true';
        currentTeacherName = selectedOption.dataset.teacher;
        updateTeacherInfo();
    }

    function updateTeacherInfo() {
        if (!teacherInfoEl) return;
        
        if (currentTeacherName) {
            teacherInfoEl.innerHTML = `<i class="bi bi-person-badge"></i> Teacher: <strong>${currentTeacherName}</strong>`;
            teacherInfoEl.style.display = 'block';
        } else {
            teacherInfoEl.style.display = 'none';
        }
    }
    ...
    saveBtn?.addEventListener("click", async () => {
        ...
        for (const [studentId, status] of changes) {
            try {
                ...
                const res = await fetch(`../php/mark_admin_attendance.php?bid=${getBrowserInstanceId()}`, {
                    method: "POST",
                    body: formData
                });
    ...
    async function fetchMonthlyAttendance(year, month, classId) {
        try {
            const cacheKey = `monthly_${year}-${month + 1}-${classId}`;
            ...
            const res = await fetch(`../php/get_admin_monthly_attendance.php?bid=${getBrowserInstanceId()}&start_date=${firstDay}&end_date=${lastDayStr}&class_id=${classId}`);
            const data = await res.json();
    ...
    async function renderSummaryTable() {
        ...
        if (selectedClass) {
            summaryIsWeekendClass = isWeekendClass(selectedClass.class_name);
            summaryTeacherName = selectedClass.teacher_name;
            
            // Update summary teacher info if exists
            const summaryTeacherInfo = document.getElementById('summaryTeacherInfo');
            if (summaryTeacherInfo) {
                if (summaryTeacherName) {
                    summaryTeacherInfo.innerHTML = `<i class="bi bi-person-badge"></i> Teacher: <strong>${summaryTeacherName}</strong>`;
                    summaryTeacherInfo.style.display = 'block';
                } else {
                    summaryTeacherInfo.style.display = 'none';
                }
            }
        } else {
            summaryIsWeekendClass = false;
            summaryTeacherName = null;
        }
        ...
        students.forEach(student => {
            ...
            nameLink.addEventListener('click', (e) => {
                e.preventDefault();
                showStudentDetails(student.id, student.full_name, attendanceData, year, month);
            });
    ...
    // === PIE CHART FUNCTIONS ===
    function showStudentDetails(studentId, studentName, attendanceData, year, month) {
